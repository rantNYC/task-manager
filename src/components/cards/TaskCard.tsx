      ...
      <div className="mt-2 grid grid-cols-2 gap-2 text-xs w-fit text-gray-300">
        ...
        {task.priority && (
          <div className="flex flex-col">
            ...
            <span className={`mt-1 inline-block rounded bg-gray-800 px-2 py-1 text-[${task.priority.color}]`}>
                {task.priority.name}
             </span>
          </div>
        )}
      ...
      <div className="flex items-center justify-between pt-4">
        <span
          className={`text-xs font-medium ${
            task.status?.role === 'completed' ? 'text-green-400' : 'text-gray-500'
          }`}
        >
          {task.status?.role === 'completed' ? `Completed ${task.completed_at ? task.completed_at.toLocaleString() : ''}` : 'Pending'}
        </span>
